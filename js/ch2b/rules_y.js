...
/* ⚠ **0〜100の評判スコアは書かない**（作者指摘 8/9）。第2章の上のバーは番付(800点)で、
     内部の評判(G.rep)はプレイヤーに見えない。見えない数字の式（評判×30円・−5点・7日平均）を
     言っても検算できない。料金は【運営→料金】の「目安」＝画面に出ている数字で語る。
   ⚠ **1本＝1つのことだけ。前のカードを読んでいる前提にしない**（S級審査1回目・8/9）。
     順不同で1日1本なので、「それ」「ここだけ」「見ただろう」等の指示語は禁止 */
/* ⚠ 収容系の「1台で客◯人」は**同時ではなく1日あたり**（scaleScore の per＝
     「1席で1日に何人まで無理なく捌けるか」）。「1日に」を必ず付けて統一する（審査2回目） */
/* ⚠ **台詞は作者の最終稿（2026-08-09）を逐語で写したもの。**勝手に整えない。
     旧#6（バイト60人比）・#16（小学生以下お断り）・#17（サウナ定員）・
     #21（水風呂定員）・#27（愛想）は作者判断で廃止（idは欠番のまま残す） */
const NAPPA_ADVICE_Y = [
  /* ── 導線 ── */
  { id: 1, cat: 'dosen', text: '客が「遠回りだ」と怒るのは、入口から出口まで歩いた総距離だ。回り道のない導線にしろ' },
  { id: 2, cat: 'dosen', text: '導線の採点は4区間だ。入口→洗い場、風呂→サウナ、サウナ→水風呂、水風呂→イス。各3マス以内にしろ' },
  { id: 3, cat: 'dosen', text: '導線の採点は、各区間3マスで満点、4マスで半分、5マスで0点だ。たった1マスで点が消えるぞ' },
  { id: 4, cat: 'dosen', text: 'サウナ客は歩かされるのを嫌う。普通の客より3割厳しい目で見てるぞ' },
  /* ── 清潔 ── */
  { id: 5, cat: 'clean', text: '清潔度は階ごとの平均だ。バイト不在の階を1つ作るだけで、評判はガタ落ちだ', req: () => (CONF.areas || []).length > 2 },
  { id: 7, cat: 'clean', text: '汚れは90分放っておくと、クレームのもとになる。真っ黒になる前に拭かせろ' },
  { id: 8, cat: 'clean', text: 'バイトは持ち場のフロアしか見ない。各階1人ずつ置け。無人の階は汚れたままだ', req: () => (CONF.areas || []).length > 2 },
  /* ── 料金 ── */
  { id: 9, cat: 'fee', text: '料金画面の「目安」が、客の納得ラインだ。1円でも超えたらコスパは0点だぞ' },
  { id: 10, cat: 'fee', text: '赤字にならない範囲で、入館料は目安の1割下にしろ。それでコスパ満点だ' },
  { id: 11, cat: 'fee', text: 'あんたの店が良くなれば、料金の「目安」も上がる。値上げしたけりゃ、先に店を磨け' },
  { id: 12, cat: 'fee', text: '値下げしすぎて赤字になると、コスパの点も落ちる。安けりゃいいってもんじゃない' },
  /* ── 経費・客層 ── */
  { id: 13, cat: 'keihi', text: 'タオルの経費は貸した枚数×60円だ。無料貸出は客が増えるが、洗濯代もまるごと増える' },
  { id: 14, cat: 'keihi', text: 'シャンプーとボディソープは、合わせて客1人10円かかる。化粧水・乳液は合わせて5円だ' },
  { id: 15, cat: 'kyaku', text: '子供向けアイテムを置くと、子供料金の「目安」が上がるぞ' },
  /* ── サウナ・風呂・水風呂 ── */
  { id: 18, cat: 'sauna', text: '熱さの好みは客ごとに違う。温度帯の違うサウナを2つ持て。1つじゃ客の好みを取りこぼす' },
  { id: 19, cat: 'sauna', text: '女湯のサウナは上限85℃が目安。熱すぎるサウナは嫌われる', req: () => yBuilt(AY.ONNA) },
  { id: 20, cat: 'furo', text: 'ぬる湯・あつ湯・超あつ湯。この3つはマストだ。' },
  { id: 22, cat: 'mizu', text: '水風呂は14℃以下がキンキン、20℃超えがぬるめだ。両方置けば、サウナ客全員がととのえる。' },
  { id: 23, cat: 'furo', text: '炭酸泉と電気風呂は、湯温で文句を言われん。ぬる湯の枠に使え' },
  /* ── ととのい・サービス ── */
  { id: 24, cat: 'totonoi', text: '安いベンチだけでは、ととのえない。ととのい点の半分は、イスの質で決まるぞ' },
  { id: 25, cat: 'totonoi', text: 'ととのいイスの種類を増やせ。寝るのと座るのは別の欲だ' },
  { id: 26, cat: 'service', text: '脱衣所に洗面所・体重計・扇風機は絶対に置け。' },
  { id: 28, cat: 'service', text: 'アメニティの相場は¥100。ぼったくりはほどほどに。' },
  { id: 29, cat: 'service', text: '壊れた設備はすぐ直せ。それだけで評価が上がる。', req: () => (G.equip || []).some(e => e.cond <= 0) },
  { id: 30, cat: 'service', text: '垢すりタオルは置いたか？ジジイが喜ぶ', req: () => !(typeof hasAkasuri === 'function' && hasAkasuri()) },
  { id: 31, cat: 'service', text: 'サウナマットは置いたか？サウナー必須アイテムだ', req: () => !(typeof hasMat === 'function' && hasMat()) },
  /* ── 収容・データ・番付 ── */
  { id: 32, cat: 'capa', text: '靴箱＝施設のキャパだ。客を増やしたければ、靴箱を増やせ' },
  { id: 33, cat: 'data', text: '【データ】画面を見ろ。あんたのやるべきことは、全部そこに書いてある' },
  { id: 34, cat: 'banzuke', text: '番付スコアをよく見ろ。持ってない部門は0点だ。飯は5階の食堂を建てるまで0のままだぞ', req: () => G.ch2 && G.ch2.battleKnown },
  { id: 35, cat: 'banzuke', text: '全部そこそこ、が実は強い。強みを伸ばす前に、0点の部門を潰せ', req: () => G.ch2 && G.ch2.battleKnown },
  { id: 36, cat: 'banzuke', text: '番付首位はSAUNA GATE 37、650点。あんたは超えられるか？', req: () => G.ch2 && G.ch2.battleKnown },
  { id: 37, cat: 'banzuke', text: '番付800点と大会の100点は別物だ。大会は日ごろの実績・予選・市民投票・最終戦の合計だぞ', req: () => G.ch2 && G.ch2.battleKnown },
  /* ── キャラ出し（作者直筆 8/9）＝攻略ではなく熱波太郎という人間を出す日 ── */
  { id: 38, cat: 'chara', text: '俺のサウナデビューは32歳。場所は群馬の草津温泉だった' },
  { id: 39, cat: 'chara', text: '俺のホームサウナ？そいつは内緒だ。混んだら困るからな' },
  { id: 40, cat: 'chara', text: '人生がツラい？大丈夫。俺たちにはサウナがある' },
  { id: 41, cat: 'chara', text: '☆5評価待ってるぜ' },
  { id: 42, cat: 'chara', text: '「オート修理」めっちゃ便利だぞ。今すぐ課金しろ。' },
...
/* 今日のアドバイスを選ぶ（同じ日は選び直さない＝キャッシュ）。
   ・直近10本（advSeen）は出さない
   ・**同じカテゴリは3日空ける**（advCats＝直近2日ぶんの cat・審査2回目）＝
     「導線→導線→導線」と同じ話が続くと、文面が違っても飽きられる */
function yTodayAdvice() {
  ...
  const cats = G.ch2.advCats || [];
  const okReq = a => !a.req || a.req();
  const fresh = NAPPA_ADVICE_Y.filter(a => okReq(a) && !seen.includes(a.id));
  // 同カテゴリ連投を避ける。避けた結果ゼロになるなら、カテゴリの縛りだけ諦める
  const spread = fresh.filter(a => !cats.includes(a.cat));
  const pool = spread.length ? spread : fresh.length ? fresh : NAPPA_ADVICE_Y.filter(okReq);
  const picked = pool.length ? pick(pool) : null;
  G.ch2.advDay = G.day;
  G.ch2.advId = picked ? picked.id : null;
  if (picked) {
    G.ch2.advSeen = [...seen, picked.id].slice(-10);
    G.ch2.advCats = [...cats, picked.cat].slice(-2);
  }
  return picked;
}
/* 熱波太郎の一言（外観図パネル）を描く。
...
const Y_INTRO_Y = [
  ...
  /* ⚠ 全5場面は **作者による全面改稿（8/10）**。一文ずつ短く切って読ませる形になった。
     台詞の中の \n は、吹き出しの中でそのまま改行される（#storyText は white-space:pre-wrap）。
  ...
  { art: 'y_living', lines: [
    { narr: true, text: 'ととのい市で所帯を持った。' },
    { narr: true, text: '妻の奈津とは社内結婚。' },
    { narr: true, text: '休みの日は、二人で市内のサウナを開拓した。' },
    { narr: true, text: 'その度に、「サウナノート」にメモをした。' },
    { narr: true, text: '導線、サウナの種類、水風呂の温度。' },
    { sp: '奈津', text: '水風呂、十六度だった。あなた、十七って書いてたけど。' },
    { sp: '俺', text: '……直しとく。' },
    { narr: true, text: 'いつからか、二人のメモになっていた。' },
  ]},
  { art: 'y_intro_minato', lines: [
    { narr: true, text: 'ある夜。観覧車が海に映る、外気ベイの岸だった。' },
    { sp: '俺', text: 'なあ……俺たちのサウナを作らないか？\n熱波銀座に、良い物件があるんだ。' },
    { sp: '奈津', text: '知ってる。\nあなた、あの物件の前で三回、足を止めてた。' },
    { sp: '俺', text: '……見てたのか。' },
    { sp: '奈津', text: '帳簿は私。設備はあなた。\nそれでよければ、契約しようか。' },
    { sp: '俺', text: '奈津、ありがとう。' },
    { narr: true, text: '物件の契約を済ませた。' },
  ]},
  { art: 'y_tenku_meshi', lines: [
    { narr: true, text: '契約の帰り、サウナバトル六連覇中の王者「SAUNA GATE37」に訪れた。' },
    { narr: true, text: '風呂を上がって、食事処で落ち合う。' },
    { sp: '俺', text: 'ととのった。\nさすが、サウナバトル王者だ。' },
    { sp: '奈津', text: '言われなくても、顔に書いてある。' },
    { sp: '奈津', text: '私たちも出るの？\nサウナバトルに。' },
    { sp: '俺', text: 'もちろんだ。' },
    { sp: '奈津', text: 'この店に勝てるの？\n六連覇王者でしょ。' },
    { sp: '俺', text: '俺たちなら、出来るさ。' },
