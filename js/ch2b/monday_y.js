...
/* ============ 買い出し ============
   ...
   買い出しに出た日の3回に1回くらい、店の役に立たないものを買って帰る。

   ⚠ **どれもフラグを持たない＝何度でも出る**（作者指定 8/14）。
     一度きりの物語を足すと、長く遊ぶ人ほど早く涸れる。
     ここは「また同じ物を買っている」で笑える枠なので、**使い回しでいい。**
     `mood` を書くと、その日ぶんだけ妻の機嫌が動く（高い買い物ほど下がる）    */
const MON_MUDA_Y = [
  ...
  { name: '信楽焼のたぬき（等身大）', cost: 14000, mood: -5,
    say: '「玄関に置きな」と言われて頷いた。うちの玄関は、このたぬきより狭い。' },
  { name: '業務用のかき氷機', cost: 28000, mood: -8,
    say: '夏に売れると踏んだ。買ったのは十一月だった。' },
  { name: '鮭をくわえた熊の木彫り', cost: 7800,
    say: '鮭の目が、やけにこちらを見る。脱衣所には置けない。' },
  { name: '「ととのう」ご当地キーホルダー 百個', cost: 8000,
    say: '一個八十円。安い。渡す相手が、百人いない。' },
  { name: '柄がばらばらのサウナハット 三十個', cost: 18000, mood: -4,
    say: '揃いのつもりが、在庫の寄せ集めを渡された。三十人が三十様の頭になる。' },
  { name: '巨大な招き猫の貯金箱', cost: 4200,
    say: '小銭を入れると鳴る。その晩、誰も入れていないのに一度鳴った。' },
  { name: '割り箸 一万膳', cost: 6500,
    say: '五階の食堂で使うつもりだった。一日五十膳としても、二百日ぶんある。' },
  { name: '中古の卓球台', cost: 22000, mood: -8,
    say: '昔の健康ランドには、たいてい置いてあった。うちに置く場所は、無い。' },
  { name: '「電気風呂」のホーロー看板', cost: 9800,
    say: 'うちに電気風呂は無い。看板のほうが先に来てしまった。' },
  { name: '足つぼマット（特大）', cost: 5600,
    say: '試しに乗ったら、声が出た。客に乗せるのは、やめておく。' },
  { name: '業務用のミキサー', cost: 12000, mood: -4,
    say: '何を混ぜるのか、店を出るまで考えていた。まだ答えは出ていない。' },
  { name: 'ラムネ 五十本', cost: 7000,
    say: '瓶が涼しげだったので買った。栓を開ける道具を、買い忘れた。' },
  { name: '中古のジュークボックス', cost: 35000, mood: -12,
    say: '昭和の曲しか入っていない。そこはいい。動かないのが困る。' },
  { name: '「快眠」と書かれた枕 十個', cost: 6800,
    say: '仮眠室ができたら使う、と自分に言い聞かせた。仮眠室の予定は、まだ無い。' },
];
function yMondayMuda() {
  /* **3回に1回**（作者指定 8/14・以前は6回に1回）。
     買い出し自体が定休日の2〜3割なので、6回に1回では**百日遊んで五回**しか出ず、
     せっかくの枠がほとんど働いていなかった                                  */
  if (Math.random() >= 0.33) return '';
  const m = MON_MUDA_Y[Math.floor(Math.random() * MON_MUDA_Y.length)];
  if (G.cash < m.cost) return '';
  G.cash -= m.cost;
  if (m.mood && typeof yMoodAdd === 'function') yMoodAdd(m.mood, m.name);
  if (typeof log === 'function') log('🛍 つい買った：' + m.name + '（' + yen(m.cost) + '）');
  return '<br><br>そして、<b>「' + m.name + '」</b>を買った。'
       + yen(m.cost) + '。'
       /* ⚠ **ゲージを使わない章では数字を出さない。**いまの第2章は `gauges: false`＝
          `yMoodAdd` が素通りするので、書くと「下がる」と言って下がらない嘘になる
          （出来事のほうは `yStripGauge` が同じ役目をしている）              */
       + ((m.mood && CONF.gauges) ? '<span class="opt-sub">（妻の機嫌 ' + m.mood + '）</span>' : '')
       + '<br><span class="opt-sub">' + m.say + '</span>';
}
